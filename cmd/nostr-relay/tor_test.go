	...
	go func() {
		...
		for {
			...
			case strings.HasPrefix(line, "AUTHCHALLENGE SAFECOOKIE "):
				...
				serverHash := safeCookieHash("Tor safe cookie authentication server-to-controller hash", cookie, clientNonce, serverNonce)
				expectedCookieAuth = "AUTHENTICATE " + hex.EncodeToString(safeCookieHash("Tor safe cookie authentication controller-to-server hash", cookie, clientNonce, serverNonce))
